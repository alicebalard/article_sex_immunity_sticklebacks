...
#############################################################
## Calculate DMS/DMR accounting for covariates: brotherPairID

## Per brother pair:
getDMperBP <- function(BP){
  ## Unite object for one treatment:
  metadataBP_CC = fullMetadata_OFFS[fullMetadata_OFFS$brotherPairID %in% BP &
                                         fullMetadata_OFFS$trtG1G2 %in% c("NE_control"), ]
  metadataBP_CT = fullMetadata_OFFS[fullMetadata_OFFS$brotherPairID %in% BP &
                                         fullMetadata_OFFS$trtG1G2 %in% c("NE_exposed"), ]
  metadataBP_TC = fullMetadata_OFFS[fullMetadata_OFFS$brotherPairID %in% BP &
                                         fullMetadata_OFFS$trtG1G2 %in% c("E_control"), ]
  metadataBP_TT = fullMetadata_OFFS[fullMetadata_OFFS$brotherPairID %in% BP &
                                         fullMetadata_OFFS$trtG1G2 %in% c("E_exposed"), ]
  
  ## Make 4 separate uniteCov:
  myuniteCovBP_CC = methylKit::reorganize(methylObj = uniteCovHALF_G2_woSexAndUnknowChrOVERLAP,
                                  treatment = metadataBP_CC$trtG1G2_NUM, sample.ids = metadataBP_CC$ID)
  myuniteCovBP_CT = methylKit::reorganize(methylObj = uniteCovHALF_G2_woSexAndUnknowChrOVERLAP,
                                  treatment = metadataBP_CT$trtG1G2_NUM, sample.ids = metadataBP_CT$ID)
  myuniteCovBP_TC = methylKit::reorganize(methylObj = uniteCovHALF_G2_woSexAndUnknowChrOVERLAP,
                                  treatment = metadataBP_TC$trtG1G2_NUM, sample.ids = metadataBP_TC$ID)
  myuniteCovBP_TT = methylKit::reorganize(methylObj = uniteCovHALF_G2_woSexAndUnknowChrOVERLAP,
                                  treatment = metadataBP_TT$trtG1G2_NUM, sample.ids = metadataBP_TT$ID)
  
  ## remove bases where NO fish in this BP has a coverage
  vectUniteObj = c(myuniteCovBP_CC, myuniteCovBP_CT, myuniteCovBP_TC, myuniteCovBP_TT)
  for (i in vectUniteObj){
    i = methylKit::select(i, which(!is.na(rowSums(percMethylation(i)))))
  }
